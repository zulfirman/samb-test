...
func SaveReleaseData(c echo.Context) error {
	var body struct {
		Header    model.T_goods_release_field    `json:"header"`
		GoodsList []model.T_goods_release_detail `json:"goodsList"`
	}
	err := c.Bind(&body)
	if err != nil {
	...
	body.Header.TrxOutNo, err = getTrxOutNumber(tx)
	if err != nil {
		return zhelper.RsMessage(c, 500, err.Error())
	}

	saveModel := model.T_goods_release{
		T_goods_release_field: body.Header,
		GoodsDetail:           body.GoodsList,
	}

	if err := tx.Save(&saveModel).Error; err != nil {
		tx.Rollback()
		return zhelper.RsMessage(c, 500, "Failed to save goods release: "+err.Error())
	}

	// Collect all product IDs and warehouse IDs from the goods release
...
func getTrxOutNumber(tx *gorm.DB) (string, error) {
	var latestTrx model.T_goods_release
	if err := tx.Order("id DESC").First(&latestTrx).Error; err != nil {
		...
		latestTrx.TrxOutNo = "00000"
	}

	// Increment the latest TrxInNo
	latestTrxNo, err := strconv.Atoi(latestTrx.TrxOutNo)
	if err != nil {
		tx.Rollback()
